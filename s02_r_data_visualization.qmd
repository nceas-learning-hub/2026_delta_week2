...
1. **`ggplot()`** opens the plot (note, the **package** is called `ggplot2` but the function is just `ggplot()`.  What happened to the original `ggplot`? [it is still out there](https://github.com/hadley/ggplot1){.external target="_blank"}, but really only for historical interest!)
...
Try searching the following table for only specific locations or visitor types, and sorting by date or number of visits. 
