...
Make sure you’re in the right project (`training_<username>`). Use Git Pull to check for any changes. Then, create a new Quarto document, delete the default text below the YAML header, and save it as **`data-visualization.qmd`**.
...
![Each column mapped in `aes()` (left) becomes a visual property in the plot (right), with a corresponding guide (axis or legend). *Note: figure uses a different dataset for illustration.*](images/r_data_visualization/ggplot-data-to-viz.png){.lightbox width=80% fig-align="center"}
:::
