...
The four components above are entry points to a richer underlying framework called the **Grammar of Graphics**. A quick look at this framework will help you reason about what's possible, and what to change when plots don't look right.

::: callout-exercise

Before looking at the figure below, predict what you'd see if you mapped these columns from a salmon escapement dataset:
...
How many legends would appear? What would each one show?

::::{.callout-answer}

:::{.column-body-outset}
![Each column mapped in `aes()` (left) becomes a visual property in the plot (right), with a corresponding guide (axis or legend). *Note: figure uses a different dataset for illustration.*](images/r_data_visualization/ggplot-data-to-viz.png){.lightbox width=100% fig-align="center"}
:::

Trace the arrows from the data table to the plot: each `aes()` mapping is one arrow. `ggplot2` executes the full Grammar of Graphics pipeline behind the scenes:
