...
Layer multiple geometries exactly like regular `ggplot2`.  Here it is best to separately assign each `data =` and `mapping = aes()` inside each `geom_sf` call.  Later geoms are visually stacked on top of earlier geoms.  Let's read in and add rivers to the plot (already in EPSG:3338), along with cities from our `pop_3338_sf` data:
